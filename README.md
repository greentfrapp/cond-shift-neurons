...
There are some implementation differences, eg. learning rate, number of convolutional layers etc. Refer to **[Implementation Details and Differences](#implementation-details-and-differences)** below for details.
...
This experiment was not in the paper but was used by Finn et al. ([2017](https://arxiv.org/pdf/1703.03400)) to demonstrate metalearning regression. Decided to try it out with CSN.

Run the following to train CSN on the sinusoid toy experiment, as described in Section 5.1 of Finn et al. ([2017](https://arxiv.org/pdf/1703.03400)).

```
$ python main.py --train --datasource=sinusoid
```

Default task parameters:

- Number of training samples per task `--num_shot_train=10`
- Number of test samples per class per task `--num_shot_test=10`

Default training parameters:

- Number of metatraining iterations `--metatrain_iterations=20000`
- Batchsize per metatraining iteration `--meta_batch_size=32`
- Metalearning rate `--meta_lr=0.0003`

Run this to test the trained model and plot the result:

```
$ python main.py --test --datasource=sinusoid
```

<p align="center"><img src="images/sinusoid.png" alt="Sample graph that shows output of regression network before and after shift." width="500px" height="whatever" align="center" style="display: block; margin:auto;"></p>

*Sample graph that shows output of regression network before and after shift.*

In the above sample, the **after shift** plot is generated by testing with `x = np.arange(-5., 5., 0.2)` samples and plotting these 50 points against the predicted output from the trained model. This means a different *shift* is used for each point and the combination of all the shifts give the resulting curve. Refer to the **[Summary](#summary)** below for a detailed explanation. Implementation details are documented [below](#adaffn-for-sinusoid-toy-task).

There is also a pretrained model you can use for testing:

```
$ python main.py --test --datasource=sinusoid --savepath=pretrained_sinusoid
```

### Omniglot

First, follow the instructions [here](data/omniglot_resized/resize_images.py) to download, sort and resize the Omniglot dataset.
...
```
$ python main.py --train --datasource=omniglot
```
...
```
$ python main.py --test --datasource=omniglot
```
...
With the smaller architecture here, I can get a ~0.96 accuracy on the test set. This is significantly lower than reported in the paper but there are many implementation differences (see [below](#adacnn-for-omniglot-5-way-1-shot)). 
...
**Does this work with regression?**

As can be seen from the sinusoid example below (and in **[Instructions](#instructions)** above), the algorithm does work reasonably well on few-shot regression.

<p align="center"><img src="images/sinusoid.png" alt="Sample graph that shows output of regression network before and after shift." width="500px" height="whatever" align="center" style="display: block; margin:auto;"></p>

One interesting thing to note is that the resulting output is somewhat discontinuous, since the output is actually made up of different *shifts* at every point. 

We can demonstrate this by explicitly plotting the separate *shifts*.

First, with a test sample of `x = -3` we calculate the resulting *shifted* network and then use this *shifted* network to classify all 50 points in `x = np.arange(-5., 5., 0.2)`. For illustration we also do the same with `x = 0` and `x = 3`.

(Note that during actual testing, we do not use a single *shift* to classify every test sample. Each test sample has its own *shift* calculated as a weighed sum of the Training Values (see above).)

<p align="center"><img src="images/sinusoid_-3.0.png" alt="Applying shift from x = -3.0 to all test samples." width="500px" height="whatever" align="center" style="display: block; margin:auto;"></p>

*Applying shift from `x = -3` to all test samples.*

<p align="center"><img src="images/sinusoid_0.0.png" alt="Applying shift from x = -3.0 to all test samples." width="500px" height="whatever" align="center" style="display: block; margin:auto;"></p>

*Applying shift from `x = 0` to all test samples.*

<p align="center"><img src="images/sinusoid_3.0.png" alt="Applying shift from x = -3.0 to all test samples." width="500px" height="whatever" align="center" style="display: block; margin:auto;"></p>

*Applying shift from `x = 3` to all test samples.*

Notice that all three results are very different and fit far more poorly to the ground truth as compared to the very first output shown above made up of different shifts. It becomes clearer when we plot everything together.

<p align="center"><img src="images/sinusoid_all.png" alt="Applying shift from x = -3.0 to all test samples." width="500px" height="whatever" align="center" style="display: block; margin:auto;"></p>
...
### adaFFN for Sinusoid Toy Task 

The paper did not demonstrate CSN on the sinusoid toy task but I thought it would be fun to try it out on a regression task. Here are the implementation details.

**Main Network**

- 2 hidden layers with 32  units and ReLU activation
- Output layer with 1 unit and no activation

This follows the architecture used for the same task by Finn et al. ([2017](https://arxiv.org/pdf/1703.03400)).

**Key Function**

- 2 hidden layers with 10 units and ReLU activation
- Output layer with 4 units and no activation
...
- The loss function is the mean squared error
	- The task test loss uses the amplitude to scale the labels and predictions before calculating mean squared error to correct for the difference in loss magnitude due to the amplitude
	- The task training loss does not do amplitude correction since this would mean that amplitude information is required to obtain the *shifts*

### adaCNN for Omniglot 5-way 1-shot

In general, a smaller architecture is used here as compared to in the paper, so that I could train the model in a shorter time. No dropout is implemented here, in contrast to the paper. The Adam optimizer also has a smaller learning rate of 3e-4, rather than 1e-3 reported in the paper (Appendix A). No gradient clipping is used.
